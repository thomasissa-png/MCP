...
**Décision tranchée E-E-A-T (`mainEntityOfPage` / `Person` / `sameAs`), ne reste plus « à arbitrer avec @ia »** :

1. **`mainEntityOfPage` : FAIT (décidé, implémentation la suite).** Ajouté sur chaque page-offre, pointant vers `url_offre`. Rationale : renforce la cohérence du graphe et la citabilité (le modèle identifie sans ambiguïté l'entité principale de la page) sans exposer la moindre donnée personnelle. Aucune tension avec le cercle fermé ni avec la revue juridique différée. Owner d'implémentation : @ia/@fullstack (édition de `jsonld.ts`, hors périmètre contenu @geo). Critère de done : chaque page-offre servable porte un `mainEntityOfPage` en top-level du graphe ou sur un noeud `WebPage`.

2. **`Person`/`author` + `sameAs` (identité de Thomas/Emmanuel) : DIFFÉRÉ, pas abandonné.** `Organization` reste, seule, l'entité éditrice exposée en JSON-LD (`organizationJsonLd`, déjà en place). Rationale de report, pas d'oubli :
   - Le pilote est en **cercle fermé** (Thomas + Emmanuel seuls détenteurs, `project-context.md` CHOIX UTILISATEUR #3) : associer un `Person`/`sameAs` nominatif à chaque offre revient à publier une identité individuelle liée à des produits financiers/crypto réglementés (Trade Republic, Kraken inclus), alors que la **revue juridique est explicitement reportée à la toute fin** (même choix utilisateur). Publier l'identité avant cet arbitrage inverserait l'ordre de décision posé par Thomas.
   - Emmanuel a un profil public identifiable (LinkedIn @EmelGoez, `project-context.md` §Base réelle d'Emmanuel) : c'est un candidat `sameAs` réel et non fabriqué le jour où la décision est prise, pas une donnée à inventer maintenant. Thomas n'a, à ce stade, aucune URL de profil public documentée : l'exposer sans confirmation serait une invention (interdit, commandement 2).
   - L'angle de différenciation de Parrainly (« vérifié par un détenteur identifié réel ») reste intact sans `Person` structuré : il est déjà porté par la Zone 6 de la page-offre (« Ce que reçoit le parrain (Thomas ou Emmanuel) »), en texte visible et dans `avantage_parrain`, sans être élevé en entité JSON-LD nominative pour l'instant.
   - **Condition de déclenchement, pas de date fixe** : implémenter `Person`/`author` + `sameAs` seulement quand (a) les URLs de profil public réelles de Thomas et Emmanuel sont validées par eux-mêmes (pas de profil inventé ni de profil non confirmé) ET (b) la revue juridique de fin de pilote est passée et n'objecte pas à l'exposition nominative. Les deux conditions sont cumulatives.
   - Owner d'implémentation le jour où les 2 conditions sont réunies : @ia/@fullstack pour le schema, @legal pour la validation finale, @geo pour vérifier l'effet sur la citabilité. Action explicitement classée **« la suite », hors clôture Phase 3**, pas un manque de ce livrable.
...
- **Re-vérification correction 2026-07-20 #2 (audit `docs/reviews/audit-phase3.md`, action 7, P2 E-E-A-T)** : `Read /home/user/MCP/project-context.md` (CHOIX UTILISATEUR #3 : cercle fermé Thomas+Emmanuel, revue juridique reportée à la toute fin ; §Base réelle d'Emmanuel : profil LinkedIn @EmelGoez confirmé, aucune URL de profil public pour Thomas documentée). Décision §5 tranchée sur cette base : 0 identité inventée, 0 `sameAs` publié avant validation par les intéressés eux-mêmes et passage de la revue juridique.

---
**Handoff → @orchestrator**
- Fichiers produits : `/home/user/MCP/docs/geo/geo-strategy.md`, `/home/user/MCP/docs/geo/monitoring-citations.md`
- Décisions prises : LLM prioritaires = les 4 déjà crawlés (ChatGPT, Perplexity, Google AI Overviews, Claude), aucun classement forcé (les 19 crawlers `AI_CRAWLERS` restent tous autorisés) ; format prioritaire = définition directe + FAQPage par offre ; claim de différenciation = date de vérification exacte + divulgation embarquée (déjà en JSON-LD, à ne pas édulcorer côté contenu) ; baseline documentée dans `test-citation-ia.md` (pré-existant, réutilisé sans re-duplication) ; **E-E-A-T (§5) tranché** : `mainEntityOfPage` = FAIT/à implémenter par @ia/@fullstack sans condition ; `Person`/`author`+`sameAs` nominatifs = DIFFÉRÉ à la conjonction (a) URLs de profil validées par Thomas/Emmanuel eux-mêmes et (b) revue juridique de fin de pilote passée, `Organization` reste seule entité éditrice d'ici là
- Points d'attention : ne pas exposer `url_parrainage` sous prétexte GEO (gouvernance miroir déjà tranchée par @ia, cf. §3) ; extension FAQ par offre et enrichissement `/divulgation` nécessitent @copywriter + @fullstack, pas @geo seul ; maillage catégorie→offre (§6 action 4) à coordonner avec @seo (pas de conflit de mots-clés identifié, juste une dépendance d'exécution) ; `docs/copy/homepage-copy.md` non touché (réservé @seo ce run) ; **implémentation `Person`/`sameAs` explicitement hors clôture Phase 3**, à réactiver par @legal/@ia quand les 2 conditions de §5 sont réunies, ne pas anticiper
