...
<!-- Version: 2026-07-20T06:00 — @data-analyst — Ajout 4 events auth P1-a (docs/ux/auth-parrain-magic-link.md) : connexion_lien_demande, connexion_lien_ouvert, session_parrain_ouverte, session_parrain_expiree. Total 23 → 27 events. Édition additive, aucune redéfinition -->

# Plan de tracking événementiel — Parrainage-IA

## Résumé exécutif

- 27 events au total : 23 consolidés depuis functional-specs.md (US-01 à US-09), 0 event manquant sur les 9 stories, propriétés enrichies (pas redéfinies) pour porter la détection d'origine IA ; + 4 events transverses auth parrain P1-a (§1, ci-dessous).
- **Ajout 2026-07-20T06:00 (auth-parrain-magic-link.md, blocker P1-a)** : 4 events intégrés pour l'authentification par lien magique de l'espace parrain (`connexion_lien_demande`, `connexion_lien_ouvert`, `session_parrain_ouverte`, `session_parrain_expiree`), proposés par @ux, non issus de functional-specs.md (mécanisme transverse à US-02/05/07/09) — cercle fermé Thomas/Emmanuel, cohérent avec le reste du document (aucune mention de Karim).
...
| `connexion_lien_demande` | Auth parrain P1-a (transverse US-02/05/07/09, `docs/ux/auth-parrain-magic-link.md` §1) | Clic sur "Recevoir mon lien de connexion" (modal Écran A) | `email_hash, resultat (envoye/non_autorise/frequence_depassee)` | PostHog | Friction de connexion espace parrain (frein adoption US-05/US-09) |
| `connexion_lien_ouvert` | Auth parrain P1-a (transverse US-02/05/07/09, `docs/ux/auth-parrain-magic-link.md` §1) | Chargement de `/parrain/verifier?token=...` (vérification serveur du token) | `resultat (succes/expire/consomme/invalide)` | PostHog | Task success du magic link (`connexion_lien_ouvert.resultat=succes` / `connexion_lien_demande.resultat=envoye`, cible ≥ 90% défaut framework, cf. auth-parrain-magic-link.md §7) |
| `session_parrain_ouverte` | Auth parrain P1-a (transverse US-02/05/07/09, `docs/ux/auth-parrain-magic-link.md` §1) | Session ouverte après vérification réussie du token, redirection vers `/parrain/tableau-de-bord` | `operateur (thomas/emmanuel)` | PostHog + Table SQL (audit accès) | Adoption effective de l'espace parrain par opérateur |
| `session_parrain_expiree` | Auth parrain P1-a (transverse US-02/05/07/09, `docs/ux/auth-parrain-magic-link.md` §1, état 12) | Action nécessitant une session valide pendant la consultation d'une page `/parrain/*`, alors que la session a expiré | Aucune propriété proposée par @ux à ce stade (event de déclenchement simple) — `[À VALIDER]` ajout éventuel de `parrain_id`/`page_courante` si granularité nécessaire, signalé et non inventé ici | PostHog | Friction de connexion espace parrain (même KPI que `connexion_lien_demande`, mesure la récurrence de reconnexion forcée) |
...
- **G7** : 0 contradiction — les 23 events issus de functional-specs.md sont repris à l'identique (Grep effectué, aucun renommage), la table SQL respecte les objets métier de product-vision.md §4, la posture RGPD reprend exactement legal-strategy.md §6a/§6b sans contredire les délais/bases légales déjà posés. Resync 2026-07-20T04:00 (checkpoint-specs-phase1.md, blocker P1-b) : retrait des 6 events périmés de l'ancien modèle onboarding V2 (`formulaire_soumission_vu`, `lien_soumis`, `lien_soumission_echec`, `lien_valide_actif`, `soumission_validee`, `soumission_rejetee`), ajout des 5 events actuels US-02/US-07 (`offre_mise_a_jour`, `offre_mise_a_jour_echec`, `offre_activee`, `offre_validee_conformite`, `offre_bloquee_conformite`), `lien_redirection_suivie`/`attribution_confirmee` passés de « proposés » à intégrés, persona parrain Karim → Thomas/Emmanuel. Ajout 2026-07-20T06:00 (auth-parrain-magic-link.md, P1-a) : 4 events transverses `connexion_lien_demande`, `connexion_lien_ouvert`, `session_parrain_ouverte`, `session_parrain_expiree` consolidés depuis @ux sans redéfinition, persona strictement Thomas/Emmanuel (aucune mention de Karim introduite), total 23 → 27 sans rupture de correspondance avec functional-specs.md (les 4 events restent explicitement identifiés comme transverses, non comptés dans le 1:1 US-01 à US-09). Personas A1/A2 (brand-platform.md §2.1/2.2) et exemple fintech (Trade Republic) restent à jour, plus aucune référence au persona obsolète ni à l'exemple hors-scope EDF. PASS sans réserve.
...
`Grep "connexion_lien_demande|connexion_lien_ouvert|session_parrain_ouverte|session_parrain_expiree" docs/analytics/tracking-plan.md` (2026-07-20T06:00) : 4/4 events présents dans le tableau §1 (une occurrence chacun dans la colonne Event), plus rappels dans le résumé exécutif et cette note de version. `Read docs/ux/auth-parrain-magic-link.md` §1 confirme que ces 4 events (et uniquement ces 4, aucun de plus) sont proposés côté UX pour l'auth parrain P1-a : consolidation fidèle, aucune propriété ajoutée qui ne figure pas déjà dans le document source (`session_parrain_expiree` laissé sans propriété, conforme à l'absence de propriété proposée par @ux — pas d'invention). Persona vérifié : les 4 events sont exclusivement rattachés à Thomas/Emmanuel (`operateur (thomas/emmanuel)`), aucune référence à Karim ni à un demandeur A1/A2, cohérent avec le reste du document (0 contradiction C4/C7/G7).
...
---
**Handoff → @fullstack, @infrastructure, @legal, @growth, @ux**
- Fichiers produits : `/home/user/MCP/docs/analytics/tracking-plan.md`
- Décisions prises : 27 events au total (23 consolidés sans redéfinition US-01 à US-09 + 4 transverses auth parrain P1-a ajoutés le 2026-07-20T06:00 : `connexion_lien_demande`, `connexion_lien_ouvert`, `session_parrain_ouverte`, `session_parrain_expiree`) ; `lien_redirection_suivie` (US-01) et `attribution_confirmee` (US-09) intégrés comme events réels (ex-« proposés », désormais dans functional-specs.md) ; 6 events périmés de l'ancien modèle onboarding V2 retirés ; persona parrain V1 corrigé Karim → Thomas/Emmanuel (les 4 events auth sont exclusivement Thomas/Emmanuel) ; mécanisme d'attribution = token dans le chemin d'URL + endpoint de redirection tracké + confirmation déclarative du parrain (pas de webhook enseigne disponible) ; stack = Plausible + PostHog + table SQL propriétaire d'attribution ; RGPD aligné legal-strategy.md §6.
- Points d'attention : le NSM est désormais entièrement traçable en prod (plus de gap sur `attribution_confirmee`) ; le token doit être dans le CHEMIN de l'URL, jamais uniquement en query param ; conservation différenciée entre données de tracking (13 mois) et lignes Attribution confirmées (10 ans, obligation comptable) ; @fullstack ne doit câbler QUE les 27 events du tableau §1 (les 6 events périmés US-02/US-07 ne doivent jamais être implémentés) ; les 4 events auth P1-a doivent être câblés dès le développement de l'espace parrain (`connexion_lien_demande` à l'envoi, `connexion_lien_ouvert` sur `/parrain/verifier`, `session_parrain_ouverte` à la redirection réussie, `session_parrain_expiree` sur toute action bloquée par expiration de session) — `session_parrain_expiree` n'a pas de propriété définie, `[À VALIDER]` avec @ux si un besoin de granularité (parrain_id, page courante) apparaît en usage réel.
