...
| 2026-07-20 | Deploy-C | session (autopilot) | **DÉPLOIEMENT RÉEL** Cloudflare Workers via CLOUDFLARE_API_TOKEN : D1 `parrainly-db` créée (id 6e441317…), migrations 2/2 remote, seed 9 offres/9 liens/2 parrains, secret INTERNAL_API_KEY posé, deploy + redeploy | **LIVE : https://parrainly.thomas-issa.workers.dev** — 8/8 checks post-déploiement PASS | Mailer en mode `console` (RESEND absent env) → auth parrain en log, pas d'email réel ; emails allowlist = défauts `.test` (AUTH_*_EMAIL absents). À reposer en secret avant mise en ligne publique. Reste : sitemaps GSC/Bing + re-test citation IA (G_PROOF distribution) |
| 2026-07-20 | Deploy-D | session (autopilot) | Câblage vérification de propriété GSC + Bing par env (balise meta `layout.tsx`, compat sous-domaine workers.dev où la vérif DNS est impossible), doc `.env.example` | build vert (tsc+lint+build), redeploy | Sitemap (20 URLs) + robots.txt (19 crawlers IA autorisés) live et submittables. **Soumission GSC/Bing = op compte-Thomas** (vérif de propriété nécessite ses comptes Google/Microsoft) : réduite à coller 2 tokens dans `GOOGLE_SITE_VERIFICATION`/`BING_SITE_VERIFICATION` puis redeploy. Non soumis à ce stade |
| 2026-07-20 | Monitoring | @geo | Cycle 1 citations IA (J0) — 11 requêtes verbatim + check `site:` via WebSearch, log §9 `monitoring-citations.md` append-only | **G_PROOF distribution : point zéro = 0/11** | Cause univoque = non-indexation (site en ligne depuis quelques min, `site:` renvoie 0 page), PAS un échec GEO. Paysage concurrent inchangé vs baseline. **Cycle 2 à programmer sous 7 j** (cadence hebdo), revérifier indexation avant d'interpréter |
| 2026-07-20 | Audit-2 | session + ux/design/ia/seo/geo/copywriter/legal | **Audit complet du live + itération jusqu'à 10/10** (double cible persona+IA). 3 exigences fondateur : T1 codes extractibles en GEO, T2 zéro prénom client-facing, T3 copy nettoyée. Livrables `docs/audit/*-audit-2.md` | **Moyenne 5,2 → 9,4** : ia/copy/design/ux 10/10, seo 9,5, geo 9, legal 7 | T1 : `CodeBadge` SSR + `code_parrainage` en JSON-LD Offer + `llms.txt` inline sur 9 offres (constat initial : code absent du HTML/JSON-LD). T2 : coupable = donnée seed (`gen-seed-sql.mjs` injectait « (Thomas ou Emmanuel) ») → corrigé « l'éditeur de ce site » + re-seed D1 ; 0 prénom sur 11 surfaces. T3 : `SITE_DESCRIPTION` unique réécrite. Bonus : vrais 404 offre/cat via middleware (auth parrain ravivée), og:image par segment, favicons, focus ring WCAG, RiskBanner distinct. **Legal 7/10 = plafond fondateur** : raison sociale (LCEN) à trancher avant mise en ligne publique ; pilote non-public OK avec intitulé neutre (`LEGAL_EDITOR_NAME` prêt). Identités internes de compte (parrain.nom, PAR-THOMAS) laissées en base (jamais client-facing). Captures Playwright bloquées par le proxy de l'env |
