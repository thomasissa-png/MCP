...
| @legal | 2026-07-20 | docs/legal/textes/ (11 items checklist §7), docs/legal/fiches-conformite/ (9 programmes), docs/legal/statut-psca-kraken-meria.md | P1-c levé (blocker mise en ligne publique) : 11 textes juridiques rédigés (CGU, confidentialité, mentions légales, cookies CNIL, divulgation, mentions de risque financier/crypto, non-affiliation, divulgation embarquée JSON, checklist plateformes IA) ; fiches de conformité CGU produites pour les 9 programmes réels avec verdict par programme, **2 verdicts NON confirmés (Trade Republic et Kraken interdisent explicitement la diffusion publique du lien, sourcé WebSearch)**, 6 À VÉRIFIER, 1 signal favorable (Dougs) ; statut PSCA/MiCA Kraken et Meria vérifié POSITIF (agréés en 2026, risque pénal §2c écarté) mais n'efface pas la clause de diffusion Kraken | 6 WebSearch de recoupement CGU réelles (au-delà de l'analyse générale de legal-strategy.md) ; découverte structurante non anticipée : Trade Republic ET Kraken interdisent contractuellement et explicitement la diffusion publique, ce qui bloque leurs 2 fiches (dont Trade Republic = priorité 1 dans la base) en l'état actuel du modèle site public + citation IA, décision produit à trancher par Thomas/Emmanuel avant mise en ligne de ces 2 fiches précisément |
| @seo | 2026-07-20 | docs/seo/keyword-map.md, docs/seo/seo-strategy.md, docs/seo/audit-technique.md (+ édition docs/copy/homepage-copy.md, slot rempli) | Priorisation absolue de l'intention « parrainage {enseigne} » (9 pages offre) sur l'intention générique « meilleure {catégorie} » (terrain comparateurs, hors stratégie), fondée sur `docs/geo/test-citation-ia.md` ; audit code réel révèle 3 gaps P0 Bing (canonicals absents, `lastModified` sitemap régénéré à chaque requête sur pages non-offre = signal de spam, mot-clé "parrainage" absent des H1/title des pages catégorie et du H1 des pages offre) ; 0 donnée de volume/difficulté chiffrée (non branché, marqué `[HYPOTHÈSE]`) | Audit exhaustif du code (`layout.tsx`, `sitemap.ts`, `robots.ts`, `jsonld.ts`, `CATEGORY_META`) avant toute recommandation (règle agent) ; keyword-map calée sur les slugs réels dérivés de `slugify()` ; zéro cannibalisation SEO/GEO documentée (§5 seo-strategy.md) ; findings priorisés P0/P1/P2 en verbe+objet+critère pour @fullstack |
...
| @geo | 2026-07-20 | geo-strategy.md, monitoring-citations.md (Phase 3) | 5 | 5 | 5 | 5 | 5 | **4,8/5.** Stratégie GEO ancrée dans le code réel (table §3 fichier→action, Reads `jsonld.ts`/`page.tsx` + WebSearch incumbents datés) ; angle de différenciation net (fraîcheur vérifiée datée + divulgation embarquée) non édulcoré ; anti-cannibalisation @seo tranchée sans conflit (§8) ; monitoring exécutable par Thomas/Emmanuel, requêtes recopiées à l'identique du baseline (0 invention). G_PROOF réel (Reads + WebSearch). |
| @seo | 2026-07-20 | keyword-map.md, seo-strategy.md, audit-technique.md (Phase 3) | 5 | 4 | 5 | 5 | 5 | **4,6/5.** Audit technique de haut niveau (code réel, 6 greps de preuve, findings P0/P1/P2 verbe+objet+critère), priorisation « parrainage {enseigne} » fondée sur le test empirique, 0 volume inventé. **Pénalité cohérence :** topical map seo-strategy §4 range Qonto (Finance perso) et Finary (Investissement) en désaccord avec 00-index/fiches-categories ; Finary marqué `[HYPOTHÈSE]` mais Qonto non signalé → contradiction P1 relevée au checkpoint. |
| @copywriter | 2026-07-20 | fiches-categories.md, faq-enrichie.md (Phase 3) | 5 | 5 | 5 | 5 | 5 | **4,8/5.** Mapping programme/catégorie repris à l'identique de 00-index (9/9, non réinventé) ; posture non-conseil stricte sur 6 catégories + 18 Q/R ; question « meilleur parrainage » traitée par la négative (légitime) ; 0 montant inventé, 0 tiret cadratin client-facing, 6 H1 alignés keyword-map §3. Structure AEO (chapô factuel + Q/R citables) non copiable par les incumbents SEO-first. |
...
**[CHOIX UTILISATEUR — 2026-07-20 #4] Identité éditeur + retrait des noms (Thomas, AUTORITÉ) :**
1. **Zéro « Thomas »/« Emmanuel » en client-facing** (site public). Appliqué : 0 prénom sur les 11 surfaces publiques (la divulgation venait de la donnée seed, corrigée en « l'éditeur de ce site »). Les identités internes de compte (`parrain.nom`, `PAR-THOMAS`/`PAR-EMMANUEL`, emails auth) restent en base, jamais rendues au public.
2. **Éditeur légal = une SOCIÉTÉ (option A)**, dont la **création est en attente**. En attendant : pilote non-public, mentions légales/confidentialité en intitulé neutre (« société en cours de constitution »). **Activation en une étape** dès l'immatriculation : poser la dénomination sociale dans `LEGAL_EDITOR_NAME` (+ `LEGAL_PUBLICATION_DIRECTOR`, `LEGAL_HOST`) puis redéployer. Condition : ne pas rendre le site réellement public (pas d'indexation GSC/Bing, pas de campagne) tant que la société n'est pas immatriculée. `@legal` = 7/10 (contenu) → 10/10 une fois la raison sociale posée.
